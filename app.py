...
class ENP:
    ...
    def aes_encrypt(self, password, iv):
        ...
        #encrypted = cipher.encrypt(self.pad(self.ndb_val.encode("UTF-8"), AES.block_size))
        encrypted = cipher.encrypt(raw.encode())
        self.enp = base64.b64encode(iv + encrypted).decode("utf-8")
        self.iv = iv
...
def register():
    ...
    try:
            ...
            print(id1)
            print(password)
            hex_, ndb, enp, iv = ens_obj.authentication(str(password))
            enp_double = ens_obj.aes_encryption_double(password, enp, iv)
            id = ens_obj.insert_user_details(id1, name1, phone,
                                            email, password, hex_, ndb,enp_double, iv)
            # hex_, ndb, enp, iv = ens_obj.authentication(str(password))
            # enp_double = ens_obj.aes_encryption_double(user_input["password"], enp, iv)
            # id = ens_obj.insert_user_details(user_input["id"], user_input["name"], user_input["phone"],
            #                                 user_input["email"], user_input["password"], hex_, ndb,enp_double, iv)
            # id = ens_obj.insert_user_details(14, "test1", "9999900000",
            #                              "testuser@gmail.com", "abc@123", hex_, ndb,enp_double, iv)
            status = True
            remarks = str(id)
    except Exception as error:
            status = False
    ...
    return render_template("registermessage.html")
    # else:
    #         return ({'status': False, 'remarks':'Content type != application/json'})
...
#         status = False
#         remarks = error
#     return ({"status": status, "remarks": str(remarks)})
...
#         iv = res_[8]
#         print(iv)
#         hex_, ndb, enp, iv = ens_obj.authentication(password, iv,hex_, ndb)
#         enp_double = ens_obj.aes_encryption_double(password, enp, iv)
#         status = user_enp == enp_double
#         remarks = "Same Password" if status else "Password Mismatch"
#     except Exception as error:
